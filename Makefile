...
build :
	cd $(EMBENCH_DIR);\
	./build_all.py -v --cpu-mhz 1000 --clean --use-vector $(USE_VECTOR) --builddir build --arch riscv32 --chip generic --board ri5cyverilator --cc /home/brumaire/build/gnu-fpu/bin/riscv32-unknown-elf-gcc --cflags="-c -O2 -fno-tree-vectorize -ffunction-sections -march=rv32imafdv -mabi=ilp32d" --ldflags="-Wl,-gc-sections" --user-libs="-lm"
	mkdir -p $(EMBENCH_DIR)/build/bin
	$(foreach x,$(EMBENCH_BENCHMARKS), mv $(EMBENCH_DIR)/build/src/$(x)/$(x) $(EMBENCH_DIR)/build/bin/$(x);)

.PHONY: run
run :
	spike --isa=rv32imafdv --varch=vlen:4096,elen:64 ~/build/pk/riscv32-unknown-elf/bin/pk -s $(EMBENCH_DIR)/build/bin/$(BENCHMARK) ; echo $?
